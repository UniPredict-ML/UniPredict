...
const UniPredictApp = () => {
  ...
  const renderResults = () => {
    ...
    // Handle case when API returns empty recommendations array
    if (Array.isArray(recommendations) && recommendations.length === 0) {
      return (
        <div className="text-center py-12">
          ...
          <h3 className="text-lg font-semibold text-yellow-600 mb-2">No Data Found</h3>
          <p className="text-gray-600 mb-4">
            No data found for stream '{studentData.stream}' in district '{studentData.district}'.
          </p>
          <p className="text-gray-500 mb-6">Please try again with different criteria.</p>
          <button
            onClick={() => {
              setCurrentStep(0);
              setRecommendations([]);
